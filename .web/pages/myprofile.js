...
export default function Component() {
  ...
  {state.home_state.followers.map((lcbmhnqi, wslmbwuu) => (
  <VStack key={wslmbwuu} sx={{"padding": "1em"}}>
  <HStack sx={{"width": "100%"}}>
  <Avatar name={lcbmhnqi.follower_username} size={`sm`}/>
  <Text>
  {lcbmhnqi.follower_username}
</Text>
</HStack>
</VStack>
))}
  ...
  <Fragment>
  {state.home_state.tweets.map((emtkyyhe, ifdascze) => (
  <Grid key={ifdascze} sx={{"gridTemplateColumns": "1fr 5fr", "py": 4, "gap": 1, "borderBottom": "1px solid #ededed"}}>
  <VStack>
  <Avatar name={emtkyyhe.author} size={`sm`}/>
</VStack>
  <Box>
  ...
  {state.home_state.search_users.map((xmsmdhsj, ythzzxfx) => (
  <VStack key={ythzzxfx} sx={{"py": 2, "width": "100%"}}>
  <HStack sx={{"width": "100%"}}>
  <Avatar name={xmsmdhsj.username} size={`sm`}/>
  <Text>
  {xmsmdhsj.username}
</Text>
  <Spacer/>
  <Button onClick={(_e) => addEvents([Event("state.home_state.follow_user", {username:xmsmdhsj.username})], (_e), {})}>
  <AddIcon/>
</Button>
</HStack>
</VStack>
  ...
  {state.home_state.following.map((vsjcwzpe, badlcify) => (
  <VStack key={badlcify} sx={{"padding": "1em"}}>
  <HStack>
  <Avatar name={vsjcwzpe.followed_username} size={`sm`}/>
  <Text>
  {vsjcwzpe.followed_username}
</Text>
  <Spacer/>
  <Button onClick={(_e) => addEvents([Event("state.home_state.unfollow_user", {username:vsjcwzpe.followed_username})], (_e), {})}>
  <MinusIcon/>
</Button>
</HStack>
</VStack>
