...
export default function Component() {
  ...
  {state.home_state.followers.map((bpecgxij, iyxyjxoh) => (
  <VStack key={iyxyjxoh} sx={{"padding": "1em"}}>
  <HStack sx={{"width": "100%"}}>
  <Avatar name={bpecgxij.follower_username} size={`sm`}/>
  <Text>
  {bpecgxij.follower_username}
</Text>
</HStack>
</VStack>
))}
  ...
  <Fragment>
  {state.home_state.tweets.map((ylzgdwoo, zqoqhgsg) => (
  <Grid key={zqoqhgsg} sx={{"gridTemplateColumns": "1fr 5fr", "py": 4, "gap": 1, "borderBottom": "1px solid #ededed"}}>
  <VStack>
  <Avatar name={ylzgdwoo.author} size={`sm`}/>
</VStack>
  <Box>
  ...
  {state.home_state.search_users.map((lpshwclm, ulzzygzu) => (
  <VStack key={ulzzygzu} sx={{"py": 2, "width": "100%"}}>
  <HStack sx={{"width": "100%"}}>
  <Avatar name={lpshwclm.username} size={`sm`}/>
  <Text>
  {lpshwclm.username}
</Text>
  <Spacer/>
  <Button onClick={(_e) => addEvents([Event("state.home_state.follow_user", {username:lpshwclm.username})], (_e), {})}>
  <AddIcon/>
</Button>
</HStack>
</VStack>
  ...
  {state.home_state.following.map((pdagglbr, utujzpre) => (
  <VStack key={utujzpre} sx={{"padding": "1em"}}>
  <HStack>
  <Avatar name={pdagglbr.followed_username} size={`sm`}/>
  <Text>
  {pdagglbr.followed_username}
</Text>
  <Spacer/>
  <Button onClick={(_e) => addEvents([Event("state.home_state.unfollow_user", {username:pdagglbr.followed_username})], (_e), {})}>
  <MinusIcon/>
</Button>
</HStack>
</VStack>
