...
export default function Component() {
  ...
  {state.home_state.followers.map((hnqayflz, uzricubv) => (
  <VStack key={uzricubv} sx={{"padding": "1em"}}>
  <HStack sx={{"width": "100%"}}>
  <Avatar name={hnqayflz.follower_username} size={`sm`}/>
  <Text>
  {hnqayflz.follower_username}
</Text>
</HStack>
</VStack>
))}
  ...
  {state.home_state.img.map((uhlbaybm, zdbrckxf) => (
  <VStack key={zdbrckxf}>
  <Image src={uhlbaybm}/>
  <Text>
  {uhlbaybm}
  ...
  <Fragment>
  {state.home_state.tweets.map((ciukxdrz, bbomxuwv) => (
  <Grid key={bbomxuwv} sx={{"gridTemplateColumns": "1fr 5fr", "py": 4, "gap": 1, "borderBottom": "1px solid #ededed"}}>
  <VStack>
  <Avatar name={ciukxdrz.author} size={`sm`}/>
</VStack>
  <Box>
  ...
  {state.home_state.search_users.map((bvwwocfc, wmvlahro) => (
  <VStack key={wmvlahro} sx={{"py": 2, "width": "100%"}}>
  <HStack sx={{"width": "100%"}}>
  <Avatar name={bvwwocfc.username} size={`sm`}/>
  <Text>
  {bvwwocfc.username}
</Text>
  <Spacer/>
  <Button onClick={(_e) => addEvents([Event("state.home_state.follow_user", {username:bvwwocfc.username})], (_e), {})}>
  <AddIcon/>
</Button>
</HStack>
</VStack>
  ...
  {state.home_state.following.map((gzpdbneu, rqamgifx) => (
  <VStack key={rqamgifx} sx={{"padding": "1em"}}>
  <HStack>
  <Avatar name={gzpdbneu.followed_username} size={`sm`}/>
  <Text>
  {gzpdbneu.followed_username}
</Text>
  <Spacer/>
  <Button onClick={(_e) => addEvents([Event("state.home_state.unfollow_user", {username:gzpdbneu.followed_username})], (_e), {})}>
  <MinusIcon/>
</Button>
</HStack>
</VStack>
