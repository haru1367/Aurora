...
export default function Component() {
  ...
  {state.home_state.followers.map((yjcyarfd, rifegego) => (
  <VStack key={rifegego} sx={{"padding": "1em"}}>
  <HStack>
  <Avatar name={yjcyarfd.follower_username} size={`sm`}/>
  <Text>
  {yjcyarfd.follower_username}
</Text>
</HStack>
</VStack>
))}
  ...
  <Fragment>
  {state.home_state.tweets.map((iwsffwvq, xbuavvbt) => (
  <Grid key={xbuavvbt} sx={{"gridTemplateColumns": "1fr 5fr", "py": 4, "gap": 1, "borderBottom": "1px solid #ededed"}}>
  <VStack>
  <Avatar name={iwsffwvq.author} size={`sm`}/>
</VStack>
  <Box>
  ...
  {state.home_state.search_users.map((kiucimkt, fzxtejhq) => (
  <VStack key={fzxtejhq} sx={{"py": 2, "width": "100%"}}>
  <HStack sx={{"width": "100%"}}>
  <Avatar name={kiucimkt.username} size={`sm`}/>
  <Text>
  {kiucimkt.username}
</Text>
  <Spacer/>
  <Button onClick={(_e) => addEvents([Event("state.home_state.follow_user", {username:kiucimkt.username})], (_e), {})}>
  <AddIcon/>
</Button>
</HStack>
</VStack>
  ...
  {state.home_state.following.map((caszqqpq, lhgkjeka) => (
  <VStack key={lhgkjeka} sx={{"padding": "1em"}}>
  <HStack>
  <Avatar name={caszqqpq.followed_username} size={`sm`}/>
  <Text>
  {caszqqpq.followed_username}
</Text>
</HStack>
</VStack>
))}
